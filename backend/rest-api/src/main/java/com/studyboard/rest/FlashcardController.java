...
public class FlashcardController {
  ...
  public List<FlashcardDTO> getAllFlashcards(
    ...
    @RequestMapping(
            value = "/{username}/deck{deckId}/flashcards/{size}",
            method = RequestMethod.GET,
            produces = "application/json")
    public List<FlashcardDTO> getFlashcardsForRevision(
            @PathVariable(name = "username") String username, @PathVariable(name = "deckId") long deckId, @PathVariable(name = "size") int size) {
        return flashcardService.getFlashcardsForRevision(username, deckId, size).stream()
                .map(FlashcardDTO::FlashcardDTOFromFlashcard)
                .collect(Collectors.toList());
    }
