...
public class FlashcardController {
  ...
  public List<FlashcardDTO> getAllFlashcards(
    ...
    @RequestMapping(
            value = "/{userId}/deck{deckId}/flashcards/{size}",
            method = RequestMethod.GET,
            produces = "application/json")
    public List<FlashcardDTO> getFlashcardsForRevision(
            @PathVariable(name = "userId") long userId, @PathVariable(name = "deckId") long deckId, @PathVariable(name = "size") int size) {
        return flashcardService.getFlashcardsForRevision(userId, deckId, size).stream()
                .map(FlashcardDTO::FlashcardDTOFromFlashcard)
                .collect(Collectors.toList());
    }
